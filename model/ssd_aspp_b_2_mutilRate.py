...
class Aspp_b_2(nn.Module):
    ...
    """
    def __init__(self,in_planes,out_planes,rate,stride=1,scale=0.1):
        #rate 1 2 5   9
        #     2 4 10  18
        #     3 6 15  27
        ...
        self.out_channels = out_planes
        self.rate = rate
        inter_planes = in_planes // 24   # 后边这个值，考虑微调 原来是8
        if(len(rate)==3 and len(rate[0])==4):
            ...
            self.branch0_1 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=2*rate[1][0], dilation=2*rate[1][0], relu=False)
            self.branch0_2 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=3*rate[2][0], dilation=3*rate[2][0], relu=False)
            self.branch1 = nn.Sequential(
                    BasicConv(in_planes, 2*inter_planes, kernel_size=1, stride=stride),
                    BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=rate[0][1], dilation=rate[0][1], relu=False))
            self.branch1_1 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=2*rate[1][1], dilation=2*rate[1][1], relu=False)
            self.branch1_2 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=3*rate[2][1], dilation=3*rate[2][1], relu=False)  
            self.branch2 = nn.Sequential(
                    BasicConv(in_planes, 2*inter_planes, kernel_size=1, stride=stride),
                    BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=rate[0][2], dilation=rate[0][2], relu=False))
            self.branch2_1 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=2*rate[1][2], dilation=2*rate[1][2], relu=False)
            self.branch2_2 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=3*rate[2][2], dilation=3*rate[2][2], relu=False)
            self.branch3 = nn.Sequential(
                    BasicConv(in_planes, 2*inter_planes, kernel_size=1, stride=stride),
                    BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=rate[0][3], dilation=rate[0][3], relu=False))
            self.branch3_1 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=2*rate[1][3], dilation=2*rate[1][3], relu=False)
            self.branch3_2 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=3*rate[2][3], dilation=3*rate[2][3], relu=False)
            self.ConvLinear = BasicConv(24*inter_planes,out_planes,kernel_size=1,stride=1,relu=False)
            self.shortcut = BasicConv(in_planes,out_planes,kernel_size=1,stride=stride, relu=False)
            self.relu = nn.ReLU(inplace=False)
        else:
            print("error! in init, the rate is incorrect! rate is ",rate)
    def forward(self,x):
        # some thing there
        if(len(self.rate)==3 and len(self.rate[0])==4):
            ...
            print("error! in forward, intern rate is ",rate)
            return 
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes,Rate):
...
def build_ssd(phase, size=300, num_classes=21,rate="6,3,2,1"):
    ...
    base_, extras_, head_ = multibox(vgg(base[str(size)], 3), #输入，三通道图片
                                     add_extras_aspp(extras[str(size)], 1024,Rate=Rate),#vgg最后一层1024
