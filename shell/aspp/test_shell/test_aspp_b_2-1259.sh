...
weight_file=./weights/voc_aspp_b_2_1259_extra_rate
eval_folder=./eval/voc_aspp_b_2_1259_extra_rate
for((i=5000;i<=170000;i+=5000));do
    weight=$weight_file/ssd300_VOC_$i.pth
    save_folder=$eval_folder/$i
    mkdir -p $save_folder
    echo $weight
    echo $save_folder
    python eval_aspp.py --model_type="aspp_b_2" --trained_model=$weight --save_folder=$save_folder --rate="1,2,5,9"
done
