...
class SSD(nn.Module):
    ...
    def forward(self, x):
        ...
        print("the size of conf",conf.size(),"self.priors:",self.priors.size())
        if self.phase == "test":
            output = self.detect(
...
def add_extras_aspp(cfg, i, batch_norm=False,Rate=[6,3,2,1]):
    ...
    for k, v in enumerate(cfg):
        if in_channels != 'S' and in_channels != 'Sl':
            if v == 'S':  #[256, 'S', 512, 128, 'S', 256, 128, 'S',256, 128,256]
...
extras = {
    ...
    '300s':[256, 'S', 512, 128, 'S', 256, 128, 'S',256, 128,256],
    '512': [],
}
mbox = {
    '300': [6, 6, 6, 6, 6, 4],  # number of boxes per feature map location
    '512': [],
}
