...
class Aspp_b_3(nn.Module):
    """
    串联加并联的操作的aspp,每层延伸出去，相当于一个fpn,先进行conv1x1
    """
    def __init__(self,in_planes,out_planes,stride=1,scale=0.1,rate=[6,3,2,1]):
        #rate 1 2 5   9
        #     2 4 10  18
        #     3 6 15  27
        super(Aspp_b_3,self).__init__()
        self.scale = scale
        self.out_channels = out_planes
        self.rate = rate
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes,Rate=[6,3,2,1]):
        ...
        self.aspp_a_4 = Aspp_b_3(512,512,stride=1,scale=1,rate=Rate)
        self.aspp_a_7 = Aspp_b_3(1024,1024,stride=1,scale=1,rate=Rate)
...
def add_extras_aspp(cfg, i, batch_norm=False,Rate=[6,3,2,1]):
    ...
    for k, v in enumerate(cfg):
        if in_channels != 'S':
            if v == 'S':  #[256, 'S', 512, 128, 'S', 256, 128, 256, 128, 256]
                layers += [Aspp_b_3(in_channels,cfg[k+1],stride=2,scale=1,rate=Rate)]
            else:
                layers += [nn.Conv2d(in_channels, v, kernel_size=(1, 3)[flag])]
            flag = not flag
        in_channels = v
