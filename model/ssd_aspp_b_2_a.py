...
from layers import *
from data import voc, coco,voc_mid_priors,voc_most_priors
import os
"""
extra layer 中存在两个问题：
1.每次conv后没有relu激活
...
class Aspp_b_2(nn.Module):
    ...
    def forward(self,x):
        ...
        if(len(self.rate)==4):
            ...
            #print("the size of cat is:",out.size())
            out = self.ConvLinear(out)
            short = self.shortcut(x)
            #print("the size of shortcut is ",short.size(),"x is",x.size())
...
class Aspp_b_2_l(nn.Module):
    ...
    def __init__(self,in_planes,out_planes,stride=1,scale=0.1,rate=[6,3,2,1]):
        ...
        #     2 4 10  18
        #     3 6 15  27
        super(Aspp_b_2_l,self).__init__()
        self.scale = scale
        self.out_channels = out_planes
        self.rate = rate
        inter_planes = in_planes // 8   # 后边这个值，考虑微调 原来是8
        if(len(rate)==4):
            self.branch0 = nn.Sequential(
                    BasicConv(in_planes, 2*inter_planes, kernel_size=3, stride=1),
                    BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=rate[0], dilation=rate[0], relu=False)
            )
            self.branch0_1 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=2*rate[0], dilation=2*rate[0], relu=False)
            self.branch0_2 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=3*rate[0], dilation=3*rate[0], relu=False)
            ...
            self.branch3_2 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=3*rate[3], dilation=3*rate[3], relu=False)
            self.ConvLinear = BasicConv(24*inter_planes,out_planes,kernel_size=1,stride=1,relu=False)
            self.shortcut = BasicConv(in_planes,out_planes,kernel_size=3,stride=1, relu=False)
            self.relu = nn.ReLU(inplace=False)
        elif(len(rate)==3):
            self.branch0 = nn.Sequential(
                    BasicConv(in_planes, 2*inter_planes, kernel_size=3, stride=1),
            ...
            self.branch2_2 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=3*rate[0], dilation=3*rate[2], relu=False)
            self.ConvLinear = BasicConv(18*inter_planes,out_planes,kernel_size=1,stride=1,relu=False)
            self.shortcut = BasicConv(in_planes,out_planes,kernel_size=3,stride=stride, relu=False)
            self.relu = nn.ReLU(inplace=False)
        else:
                print("error! the rate is incorrect!")
    def forward(self,x):
...
def add_extras_aspp(cfg, i, batch_norm=False,Rate=[6,3,2,1]):
    ...
    flag = False
    for k, v in enumerate(cfg):
        if in_channels != 'S' and in_channels != 'Sl':
            if v == 'S':  #[256, 'S', 512, 128, 'S', 256, 128, 'S',256, 128, 'Sl',256]
                layers += [Aspp_b_2(in_channels,cfg[k+1],stride=2,scale=1,rate=Rate)]
            elif v=='Sl':
                layers += [Aspp_b_2_l(in_channels,cfg[k+1],stride=2,scale=1,rate=Rate)]
            else:
                #print("v is, in_channels is",v,in_channels)
                layers += [nn.Conv2d(in_channels, v, kernel_size=(1, 3)[flag])]
            flag = not flag
        in_channels = v
    return layers
...
extras = {
    ...
    '300s':[256, 'S', 512, 128, 'S', 256, 128, 'S',256, 128, 'Sl',256],
    '512': [],
}
mbox = {
...
def build_ssd(phase, size=300, num_classes=21,rate="6,3,2,1"):
    ...
    base_, extras_, head_ = multibox(vgg(base[str(size)], 3), #输入，三通道图片
                                     add_extras_aspp(extras[str(size)+"s"], 1024,Rate),#vgg最后一层1024
