...
class Aspp_a_1(nn.Module):
    ...
    def __init__(self,in_planes,out_planes,stride=1,scale=0.1,rate=[6,3,2,1]):
        ...
        """这里的降采样要认真考虑下
        """
        self.conv1 = BasicConv(in_planes, 2*inter_planes, kernel_size=3, stride=stride, padding=1)
        if(len(rate)==4):
...
class Aspp_b(nn.Module):
    """
    串联加并联的操作的aspp,没写完
    """
    def __init__(self,in_planes,out_planes,stride=1,scale=0.1,rate=[6,3,2,1]):
        #rate can also be [1,2,3], [3,4,5], [1,2,5], [5,9,17], [1,2,5,9],try it one by one, or just concat some of them to test the map,go go go ,just move on 
        super(Aspp_b,self).__init__()
        self.scale = scale
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes,Rate=[6,3,2,1]):
        ...
        self.aspp_a_4 = Aspp_a_1(512,512,stride=1,scale=1,rate=Rate)
        self.aspp_a_7 = Aspp_a_1(1024,1024,stride=1,scale=1,rate=Rate)
...
def add_extras_aspp(cfg, i, batch_norm=False,Rate=[6,3,2,1]):
    ...
    for k, v in enumerate(cfg):
        if in_channels != 'S':
            if v == 'S':  #[256, 'S', 512, 128, 'S', 256, 128, 256, 128, 256]
                layers += [Aspp_a_1(in_channels,cfg[k+1],stride=2,scale=1,rate=Rate)]
            else:
                layers += [nn.Conv2d(in_channels, v, kernel_size=(1, 3)[flag])]
            flag = not flag
        in_channels = v
